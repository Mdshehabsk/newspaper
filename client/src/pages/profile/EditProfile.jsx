    ...
    const FormSubmit = async (e) =>{
      ...
      if(!image){
        return toast('Please select an image', {
          ...
          position: 'top-center',
          // Styling
          style: {width:'200px',background:'red',color:'white',fontSize:'2rem'},
          className: '',
          // Custom Icon
        })
      }
      const res = await axios.put('/api/v1/user/profile/update', formData,{withCredentials: true})
      if(res.status === 200){
        toast(res.data.message, {
          duration: 2000,
          position: 'top-center',
          style: {width:'200px',background:'green',color:'white',fontSize:'2rem'},
        })
        setTimeout(() => {
          apicall()
        }, 2000);
    ...
    <div className="profile-content w-full  md:w-2/3 bg-blue-50 p-8">
        <div>
          ...
          <div className="flex flex-col items-center space-y-32 mt-12 ">
            ...
            />
            <input type="file" name='avatar' onChange={avatarUpload} className='absolute left-24 top-36  bottom-4 z-10 opacity-0 ' /><AiOutlinePlusCircle className='text-5xl text-slate-50 bg-blue-700 absolute right-0 bottom-4 w-12 h-12 rounded-full' />
            </div>
